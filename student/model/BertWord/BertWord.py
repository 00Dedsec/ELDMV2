...
class BertMoudle(nn.Module):
    def __init__(self, config, gpu_list, *args, **params):
        super(BertMoudle, self).__init__()
        self.encoder = AutoModel.from_pretrained("bert-base-chinese")
        self.fc = nn.Linear(config.getint("data","max_seq_length"), 4)
    ...
    def forward(self, data):
        ...
        re = torch.max(re, dim=1)[0] #[batch, hidden_size]
...
class Bertword(nn.Module):
    def __init__(self, config, gpu_list, *args, **params):
        ...
        self.BertMoudle = BertMoudle(config, gpu_list, *args, **params)
        self.loss = nn.CrossEntropyLoss()

    def init_multi_gpu(self, device, config, *args, **params):
        self.BertMoudle = nn.DataParallel(self.BertMoudle, device_ids=device)
    
    def forward(self, data, config, gpu_list, mode, *args, **params):
        re = self.BertMoudle(data)
