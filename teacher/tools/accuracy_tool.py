...
def get_accuracy(res):
    ...
    for a in range(0, len(res)):
        ...
        total["FN"] += res[a]["FN"]
        total["TN"] += res[a]["TN"]
    return round((total["TP"] + total["TN"]) / (total["TP"] + total["TN"] + total["FP"] + total["FN"]), 4)
...
def single_label_top1_accuracy(outputs, label, config, result=None):
    ...
    for a in range(0, len(id1)):
        ...
        it_is = int(id1[a])
        should_be = int(id2[a])
        if it_is == should_be and it_is == 1:
            result[it_is]["TP"] += 1
        elif it_is == should_be and it_is == 0:
            result[it_is]["TN"] += 1
        else:
...
def ner_label_top1_accuracy(outputs, label, config, result=None):
    if result is None:
        result = []
    id1 = torch.max(outputs, dim=1)[1]
    # id2 = torch.max(label, dim=1)[1]
