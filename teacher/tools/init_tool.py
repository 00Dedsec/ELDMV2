...
def init_all(config, gpu_list, checkpoint ,mode , *args, **params):
    ...
    logger.get_log().info("Begin to initialize dataset and formatter...")
    if(mode == "train"):
    ...
    logger.get_log().info("Begin to initialize model...")
    ...
    if len(gpu_list) > 0:
        ...
        try:
            ...
            logger.get_log().warning("No init_multi_gpu implemented in the model, use single gpu instead.")
    ...
    try:
        ...
        if mode == "test":
            ...
            logger.get_log().warning(information)
    ...
    if(mode == "test"):
        result["output_function"] = init_output_function(config)
    logger.get_log().info("Initialize done.")
