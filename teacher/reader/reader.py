    ...
    if which in dataset_list:
        ...
        if mode in ["valid", "test"]:
            ...
            try:
                ...
                logger.get_log().warning("[eval] batch size has not been defined in config file, use [train] batch_size instead.")

            try:
                shuffle = config.getboolean("eval", "shuffle")
            except Exception as e:
                shuffle = False
                logger.get_log().warning("[eval] shuffle has not been defined in config file, use false as default.")
            try:
                reader_num = config.getint("eval", "reader_num")
            except Exception as e:
                logger.get_log().warning("[eval] reader num has not been defined in config file, use [train] reader num instead.")
        ...
        logger.get_log().error("There is no dataset called %s, check your config." % which)
        raise NotImplementedError


def init_test_dataset(config, gpu_list, *args, **params):
    init_formatter(config, ["test"], gpu_list,*args, **params)
